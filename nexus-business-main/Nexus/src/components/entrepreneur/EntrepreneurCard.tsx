...
export const EntrepreneurCard: React.FC<EntrepreneurCardProps> = ({
  ...
  // Safely grab the ID whether it comes from mock data (id) or MongoDB (_id)
      ...
      <CardBody className="flex flex-col h-full">
        <div className="flex items-start">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-lg font-semibold text-gray-900 mb-1 truncate">{entrepreneur.name}</h3>
            <p className="text-sm text-gray-500 mb-2 truncate">{entrepreneur.startupName}</p>
        ...
        <div className="mt-auto pt-4 flex justify-between items-center">
          <div>
            ...
            <p className="text-sm font-medium text-gray-900">{entrepreneur.fundingNeeded || 'Undisclosed'}</p>
          </div>
          
          <div>
            <span className="text-xs text-gray-500">Team Size</span>
            <p className="text-sm font-medium text-gray-900">{entrepreneur.teamSize ? `${entrepreneur.teamSize} people` : 'N/A'}</p>
          </div>
        </div>
      </CardBody>
